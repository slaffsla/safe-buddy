...
export default function DayScreen({
  blocks, isWeekendDay, speak, onClose, onStartMission,
}: DayScreenProps) {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      const current = visibleBlocks.find(b => getBlockStatus(b) === 'current');
      if (current && blockYRef.current[current.id] != null) {
        scrollRef.current?.scrollTo({ y: Math.max(0, blockYRef.current[current.id] - 40), animated: true });
      }
    }, 150);
  ...
  return (
    <View style={s.root}>
      <View style={s.header}>
        ...
        <TouchableOpacity style={s.closeBtn} onPress={onClose} accessibilityLabel="Закрыть">
        ...
        {visibleBlocks.length === 0 ? (
          ...
          return (
            <View
              key={block.id}
              style={s.row}
              onLayout={e => { blockYRef.current[block.id] = e.nativeEvent.layout.y; }}
            >
              {/* Timeline spine */}
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.75}
              >
                <Text style={[s.cardEmoji, isCurrent && s.cardEmojiCurrent, isPast && s.cardEmojiPast]}>
                  {block.emoji}
                </Text>
                <View style={s.cardInfo}>
                  <Text style={[s.cardTitle, isCurrent && s.cardTitleCurrent, isPast && s.cardTitlePast]}>
                    {block.title}
                  </Text>
...
const s = StyleSheet.create({
  ...
  closeBtn:    { width: 40, height: 40, borderRadius: 20, backgroundColor: C.white, borderWidth: 1, borderColor: C.border, alignItems: 'center', justifyContent: 'center' },
  ...
  dot:         { width: 12, height: 12, borderRadius: 6, backgroundColor: C.green, marginTop: 18 },
  dotCurrent:  { width: 16, height: 16, borderRadius: 8, marginTop: 16, backgroundColor: C.green, borderWidth: 3, borderColor: C.greenLt },
  dotPast:     { backgroundColor: C.muted, opacity: 0.4 },
  spine:       { flex: 1, width: 2, backgroundColor: C.green, opacity: 0.35, marginTop: 4 },
  spinePast:   { backgroundColor: C.muted, opacity: 0.2 },

  card:        { flex: 1, flexDirection: 'row', alignItems: 'center', borderRadius: 14, borderWidth: 1, borderColor: C.border, padding: 14, marginBottom: 10, gap: 12 },
  cardCurrent: { borderColor: C.green, borderWidth: 2, padding: 16, marginBottom: 12 },
  ...
  startBtn:     { width: 44, height: 44, borderRadius: 22, backgroundColor: C.green, alignItems: 'center', justifyContent: 'center' },
